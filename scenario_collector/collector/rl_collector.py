...
使用纯RL模型（PPO）执行策略并收集交通场景数据。

策略类型: 纯RL策略
适用场景: 作为RL基线对比

命令示例:
    python collector/rl_collector.py

与其他脚本的区别:
    - collect_data_rl: 纯RL策略
    - collect_data_fix: 固定配时策略
    - collect_data_random: 随机策略
    - rl_rule_collector: RL+规则混合（紧急车辆/路障）
...
if __name__ == '__main__':
    ...
    base_path = base_path = path_convert(f"../../scenario_dataset/{SCENARIO_NAME}/") # 存储路径
    trip_info = path_convert(f"../../scenario_dataset/{SCENARIO_NAME}/tripinfo_rl.out.xml")
