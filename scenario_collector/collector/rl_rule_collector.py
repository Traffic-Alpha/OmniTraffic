...
结合RL模型和规则专家策略的数据收集器。
- 常规情况: 使用RL模型决策
- 特殊情况（紧急车辆/路障）: 使用规则专家决策

策略类型: RL+规则混合
适用场景: 需要处理特殊事件的交通场景

命令示例:
    MAP=France_Massy SCENE=easy_random_perturbation_barrier python collector/rl_rule_collector.py

与其他脚本的区别:
    - rl_collector: 纯RL策略
    - fixed_timing_collector: 固定配时策略
    - random_collector: 随机策略
    - rl_rule_collector: RL+规则混合（当前脚本）
    - maxq_rule_collector: MaxQ排队+规则策略
