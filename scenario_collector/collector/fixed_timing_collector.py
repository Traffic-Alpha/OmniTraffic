...
使用固定配时策略收集交通场景数据。每个相位按固定时间顺序切换。

命令示例:
    MAP=France_Massy SCENE=easy_fluctuating_commuter_barrier uv run python fixed_timing_collector.py

与其他脚本的区别:
    - rl_collector: 纯RL策略
    - fixed_timing_collector: 固定配时策略（当前脚本）
